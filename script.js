...
class Main {
  constructor() {
    ...
    Promise.all([this.loadMobilenet(), this.loadTransformer()]).then(([styleNet, transformNet]) => {
  ...
  async loadMobilenet() {
    if (!this.mobileStyleNet) {
      this.mobileStyleNet = await tf.loadGraphModel("models/inception/model.json");
    }
  ...
  async loadTransformer() {
    if (!this.separableTransformNet) {
      this.separableTransformNet = await tf.loadGraphModel("models/transformer/model.json");
    }
  ...
  async startTransfer() {
    ...
    const stylized = await tf.tidy(() => {
      return this.transformNet
        .predict([tf.browser.fromPixels(this.contentImg).toFloat().div(tf.scalar(255)).expandDims(), pipeline])
        .squeeze();
    });
